...
// Constructor
ExtensibleCognitiveRadio::ExtensibleCognitiveRadio(char *virtual_interface)
{
...
*/
  ...
  /*  printf("tx_gain_soft: %d\n", ECR->get_tx_gain_soft());
  printf("tx_modulation: %d\n", ECR->get_tx_modulation());
  printf("tx_crc: %d\n", ECR->get_tx_crc());
  printf("tx_fec0: %d\n", ECR->get_tx_fec0());
  printf("tx_fec1: %d\n", ECR->get_tx_fec1());
  printf("tx_subcarrier_tx_method: %d\n", ECR->get_tx_subcarriers());
  printf("tx_rate: %f\n", ECR->get_tx_rate());
...
void *ECR_tx_worker(void *_arg)
{
  ...
  while (ECR->tx_thread_running)
  {
    ...
    while (tx_continue)
    {
      ...
      //  printf("Required Pay Load Length: %d\n",payload_byte_length);
      //      while (nread < payload_byte_length && (ECR->tx_state != TX_STOPPED)) {
      while (nread == 0 && (ECR->tx_state != TX_STOPPED))
      {
        FD_ZERO(&fds);
...
void ExtensibleCognitiveRadio::print_metrics(ExtensibleCognitiveRadio *ECR)
{
  ...
  printf("Bytes: ");
  int x = ECR->CE_metrics.payload_len;
  if (x > 0)
  {
    for (int i = 4; i < x; i++)
    {
      printf("%x ", ECR->CE_metrics.payload[i]);
    }
  }
  printf("\n");
}
...
void ExtensibleCognitiveRadio::set_esc_channel(int channel)
{
  ...
  shared_struct = (ESC_Struct *)mmap(NULL, sizeof(ESC_Struct), PROT_READ | PROT_WRITE, MAP_SHARED, fd_shm, 0);
  send_to_esc = true;
}
