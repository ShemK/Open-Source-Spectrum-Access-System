...
//    printf("crc :%d\n",ECR->get_tx_crc());
  ...
  crts_server_addr.sin_addr.s_addr = inet_addr(np.target_ip); //10.0.0.3
  ...
  crts_client_addr.sin_addr.s_addr = inet_addr(np.crts_ip); //10.0.0.2
  crts_client_addr.sin_port = htons(8000);
  socklen_t clientlen = sizeof(crts_client_addr);


  int crts_client_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);

  // Bind CRTS server socket
  bind(crts_client_sock, (sockaddr *)&crts_client_addr, clientlen); //10.0.0.2
  ...
  fd_set read_fds;
  FD_ZERO(&read_fds);
  FD_SET(crts_client_sock, &read_fds);
  ...
  if (send_flag) {
    ...
    while(sig_terminate == 0) {
      ...
      */
      ...
      int send_return = 0;
      sendto(crts_client_sock, (char *)message, packet_number, 0,
              (struct sockaddr *)&crts_server_addr, sizeof(crts_server_addr));
    ...
    //  usleep(10000);
      ...
      FD_ZERO(&read_fds);
      FD_SET(crts_client_sock, &read_fds);
      p = select(crts_client_sock + 1, &read_fds, NULL, NULL, &timeout);
      if(p > 0) {
        recv_len = recvfrom(crts_client_sock, recv_buffer, recv_buffer_len, 0,
