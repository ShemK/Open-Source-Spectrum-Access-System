...
int main(int argc, char **argv) {
  ...
  tcp_server_addr.sin_addr.s_addr = inet_addr(np.crts_ip); //10.0.0.3
  tcp_server_addr.sin_port = htons(port);
  socklen_t clientlen = sizeof(tcp_server_addr);
  ...
  bind(tcp_server_sock, (sockaddr *)&tcp_server_addr, clientlen); //10.0.0.3
  ...
  int recv_len = 0;
  FD_ZERO(&read_fds);
  FD_SET(tcp_server_sock, &read_fds);
  int status = 0;
  status = listen(tcp_server_sock,1);

  while(sig_terminate == 0){ // while CTRL-C has not been pressed
    struct timeval timeout;
    timeout.tv_sec = 0;
    timeout.tv_usec = 1000;

    dprintf("Waiting for message\n");
    p = select(tcp_server_sock + 1, &read_fds, NULL, NULL, &timeout);
    if(p > 0) {
      printf("Connection Received\n");
      int connection_sock = accept(tcp_server_sock,
                      (struct sockaddr *) & tcp_client_addr, &clientlen);
      ...
      if(connection_sock < 0) {
        ...
        while(sig_terminate == 0) {
          ...
          FD_ZERO(&connect_fds);
          FD_SET(connection_sock , &connect_fds);
          struct timeval timeout2;
          timeout2.tv_sec = 0;
          timeout2.tv_usec = 1000;

          p = select(connection_sock + 1, &connect_fds, NULL, NULL, &timeout2);

          if(p > 0 && sig_terminate == 0 && FD_ISSET(connection_sock,&connect_fds)) {
            dprintf("Connection Socket at %d \n",connection_sock);
            dprintf("TUN Interface received bytes\n");
            recv_len = recvfrom(connection_sock, recv_buffer, recv_buffer_len, 0,
                                  (struct sockaddr *)&tcp_client_addr, &clientlen);
            if (recv_len > 0) {
              // TODO: Say what address message was received from.
              // (It's in CRTS_server_addr)
              dprintf("\nCRTS received packet containing %i bytes:\n",
                        recv_len);

              strncpy(np.target_ip,inet_ntoa(tcp_client_addr.sin_addr),20);
              ...
              sendto(connection_sock,(char *)message, message_length, 0,
                        (struct sockaddr *)&tcp_client_addr, sizeof(tcp_client_addr));
