...
//    printf("crc :%d\n",ECR->get_tx_crc());
  ...
  udp_server_addr.sin_addr.s_addr = inet_addr(np.target_ip); //10.0.0.3
  ...
  udp_client_addr.sin_addr.s_addr = inet_addr(np.crts_ip); //10.0.0.2
  ...
  bind(udp_client_sock, (sockaddr *)&udp_client_addr, clientlen); //10.0.0.2
  ...
  if (send_flag) {
    ...
    while(sig_terminate == 0) {
      ...
      */
      ...
      int send_return = 0;
      sendto(udp_client_sock, (char *)message, packet_number, 0,
    ...
    //  usleep(10000);
      ...
      FD_ZERO(&read_fds);
      FD_SET(udp_client_sock, &read_fds);
      p = select(udp_client_sock + 1, &read_fds, NULL, NULL, &timeout);
      if(p > 0) {
        recv_len = recvfrom(udp_client_sock, recv_buffer, recv_buffer_len, 0,
