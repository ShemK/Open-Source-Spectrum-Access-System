...
void Initialize_CR(struct node_parameters *np, void *ECR_p,
                   int argc, char **argv);
void *change_frequency(void *input);
...
void initialize_node_parameters(struct node_parameters *np) {
 ...
 // np->node_type = "cognitive radio";
  ...
  // network parameters
  strncpy(np->crts_ip,"10.0.10.2",sizeof(np->crts_ip));
...
int main(int argc, char **argv) {
  ...
  if(np.cognitive_radio_type == EXTENSIBLE_COGNITIVE_RADIO)
  {
    ...
    ECR = new ExtensibleCognitiveRadio((char *) "radio_interface");
  ...
  if(argc > 1) {
    if(isdigit(argv[1][0])) {
      double new_freq =  atof(argv[1]);

      np.tx_freq = new_freq*1e6;
      printf("new freq: %f \n",np.tx_freq );
      np.rx_freq = new_freq + 20;
      ECR->set_tx_freq(np.tx_freq);
   //   ECR->set_rx_freq(np.rx_freq);
    }
  }
  ...
  int create_thread = pthread_create(&freqChangeThread, NULL,
                        change_frequency,(void *)ECR);

  if(create_thread < 0) {
    printf("Listening Thread Not Created\n");
  }
...
void *change_frequency(void *input) {
  ExtensibleCognitiveRadio *ECR = (ExtensibleCognitiveRadio *)input;
  ...
  while(sig_terminate == 0 && fd > -1){
    ...
    FD_ZERO(&read_fds);
    FD_SET(fd, &read_fds);
    p = select(fd + 1, &read_fds, NULL, NULL, &timeout);

    if(p > 0) {
      read(fd, buf, 255);

      if(isdigit(buf[0])) {
        double new_freq =  atof(buf)*1e6;
        ECR->set_tx_freq(new_freq);
        printf("Frequency Set At: %f\n", new_freq);
      }
      printf("Received: %s\n", buf);
      truncate(myfifo, 0);
    }
