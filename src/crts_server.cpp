...
int main(int argc, char **argv) {
  ...
  crts_server_addr.sin_addr.s_addr = inet_addr(np.crts_ip); //10.0.0.3
  ...
  bind(crts_server_sock, (sockaddr *)&crts_server_addr, clientlen); //10.0.0.3
  ...
  while(sig_terminate == 0){
    ...
    if(p > 0) {
      ...
      if (recv_len > 0) {
        ...
        strncpy(np.target_ip,inet_ntoa(crts_client_addr.sin_addr),20);
        ...
        sendto(crts_server_sock,(char *)message, message_length, 0,
